 ...
 * 
 * 1.在Vue中如何发送异步请求
 *  1) vue-resource 插件,先安装,然后引入,声明使用插件
 *  2) axios 插件,先安装,然后引入,直接使用
 *  async + await 配合使用,简化promise
 * 
 * 2. Vue中的路由:
 * 路由:一种映射关系,前端路由:一个链接地址对应着一个路由组件,最终页面中产生了一个a标签,点击a标签,页面中指定的位置可以展示某个组件中的相关内容
 * 路由器:管理路由的,使用路由的时候需要先引入路由器对象,然后声明使用插件,创建路由器对象,并暴露出去,在main.js中需要注册路由器对象
 * 一旦注册路由器对象了,那么每个组件的实例对象中都会出现两个对象
 *  $router:路由器对象
 *  $route:路由信息对象,可以获取路由跳转的时候传递的参数及不用的传递参数的方式()
 * 路由组件:普通组件一旦通过路由进行配置及注册,那么这个组件就是路由组件了
 * 路由链接:<router-link> 是一个存储路由地址的标签,最终在页面中产生a标签(<router-link> 标签有to属性,用来存放地址的)
 * 路由视图:<router-view> 是路由地址对应的将来用来显示路由组件内容的标签,<router-view>也可以向组件内传递数据
 * 声明式路由:<router-link>配合<router-view>标签使用,不需要通过js代码就可以实现路由跳转操作
 * 编程式路由:通过$router对象调用里面的方法实现路由跳转及传参
 * 路由传参:点击路由地址,显示对应路由组件的时候可以进行传递数据的参数
 * 缓存组件(缓存的路由组件):<keep-alive>包裹的路由组件,就可以进行组件的缓存操作(当前的组件被切换了,但是依然存活着)
 * 路由可以嵌套,所以,就会出现一级路由,二级路由...
 * Vuex:集中管理状态数据的一种模式,也可以看成是一个工具
 * Vuex:可以实现组件之间传递数据(组件之间的通信方式之一)
 * state:包含了多个状态数据的对象
 * mutations:包含了多个直接修改状态数据的方法的对象(同步的代码)
 *  mutations中的每个方法都是一个mutation
 * actions:包含了多个间接修改状态数据的方法的对象(同步异步都可以)
