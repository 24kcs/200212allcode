...
export default {
  ...
  data() {
    return {
      selectedAddr: {} ,// 从年初选中后的地址信息对象的
      orderComment:'老板好帅哦,快点发货哦,发大财哦' // 订单的备注
    }
  },
  watch: {
    // val ---->userAddressList
    'tradeInfo.userAddressList': function(val) {
      // 找出默认的选中的地址
      const defaultAddr = val.find(addr => addr.isDefault === '1')
      // 判断该默认选中的地址是否存在
      if (defaultAddr) {
        // 保存起来
  ...
  methods: {
    // 提交订单的操作
    async submitOrder() {
      // 收集参数数据----订单的编号(tradeNo,)---商品信息数组(detailArrayList) ---都在tradeInfo对象中
      ...
      const orderInfo = {
        ...
        orderDetailList: detailArrayList // 商品信息对象的数组
      }
