  ...
  2. async + await  promise 的方式
      ...
      不同点:
        ...
        localStorage
          ...
          异步action,返回的结果的操作问题
          组件内部提交异步的action,进行相应的处理
          1. 异步action中返回的是空字符串或者是非空字符串,组件内部提交action,接收异步action返回的结果,判断结果是否为空串,然后进行成功或者失败的操作
          2. 一次性提交多个异步action,通过Promise.all()方法统一的处理promise
          3. 提交异步action使用try-catch的方式解决成功或者失败的操作
          4. 提交异步action,通过async-await + then的方式来进行解决成功或者失败的操作
          vee-validate----2.2.13版本,新版本使用起来没有这个版本方便---Vue表单验证操作的



          第9天

          1. 通过Vue的表单验证操作实现注册的功能
          2. 登录的时候也需要表单的验证操作----大家自己做

            

            登录的操作----cookie 和  token 或者  localStorage的 实现自动登录的方式



          userTempId: 浏览器端生成的,用于标识一个未登录的用户,每次请求的时候需要携带userTempId(尤其是购物车)
           看到的是未登录的情况下的 购物车数据
          token:请求登录的时候服务器创建并返回的用户标识,每次发送请求的时候是需要携带token的,自动携带的(访问购物车)
           看到的是登录后的情况下的 购物车数据

          没登录的时候向购物车加了数据,此时登录了,也加了购物车数据,此时看到的应该是未登录和登录后的购物车数据
           

          3. 关于 个人中心/支付/支付成功等组件及界面的显示和操作
          4. 路由守卫(导航守卫)
          5. 收尾的操作
          ...
          7. 登录成功后,跳转到我想去的目标的路由地址


          路由守卫(导航守卫):
          路由:是一种映射关系,地址和组件的之间的关系,在页面中指定位置的地址通过点击或者跳转的方式,在页面中的指定位置可以显示该地址对应的组件内容
          守卫:保护及操作,监视和控制的操作
          路由守卫:当路由地址发生变化的时候,通过路由守卫进行监视及控制的操作----回调函数

          导航守卫(路由守卫): 主要是vue-router提供的相关的针对路由的功能
          ...
          全局路由守卫  
            ...
            定义在路由器对象实例化之后,暴露之前进行定义的操作及守卫的控制

          路由独享守卫
            前置守卫
            定义在注册路由组件内部进行路由的控制及操作
          ...
          组件内守卫
            ...
            router-link 最终在页面中生成的是 a标签----(超)链接---地址

            router-view  最终在页面中 指定的位置进行占位--这个占位的地方用来展示 路由的地址对应的这个组件内容的

            router-link 频道---切换频道
            router-view---电视--显示对应的内容
...
    public  // 此文件夹下的所有资源会自动打包到项目根目录下
        index.html  // 首页页面(SPA的唯一页面)
        css  // 全局样式文件夹, 通过index.html关联上
        fonts  // 包含字体文件
        images  // mock接口用到其中的图片  < img src="/images/xxx.jpg"
    src
        api  // 接口请求相关
...
            index.js    // 包含n个接口请求函数 
        components    // 非路由组件
            Carousel/index.vue    // 首页轮播效果组件封装
            Footer/index.vue    // 底部组件
            Header/index.vue    // 头部组件
            Pagination/index.vue    // 分页组件(难)
            TypeNav/index.vue    // 商品分类列表组件(难)
        mock    // mock数据
        pages    // 路由组件及其子组件
        router  // 路由相关
        store  // vuex相关
...
    jsconfig.json  // 用于引入自定义模块用@时能有友好的提示
    package-lock.json  // 记录了下载模块的准确版本, 用于npm install下载完全一样的版本
    package.json   // 当前项目信息记录(名称/版本/依赖/打包运行命令)
    vue.config.js  // vue脚手架包装的webpack相关配置    
